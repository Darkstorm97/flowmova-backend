...
@Service
public class CloseServiceUnitService {
    ...
    public CloseServiceUnitService(
    ...
    @Transactional
    public ServiceUnitResponse close(UUID companyId, UUID serviceUnitId, AuthenticatedUser authenticatedUser) {
        ...
        if (serviceUnit.getStatus() != ServiceUnitStatus.OPEN) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Service unit must be OPEN to be closed");
        }

        serviceUnit.close(updater);
