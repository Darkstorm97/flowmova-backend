...
@Service
public class ArchiveServiceUnitService {
    ...
    public ArchiveServiceUnitService(
    ...
    @Transactional
    public ServiceUnitResponse archive(UUID companyId, UUID serviceUnitId, AuthenticatedUser authenticatedUser) {
        ...
        if (serviceUnit.getStatus() == ServiceUnitStatus.ARCHIVED) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Service unit is already archived");
        }

        serviceUnit.archive(updater);
